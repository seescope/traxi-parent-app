...
const { CongratulationsComponent, mapStateToProps } = proxyquire.noCallThru()('../../App/Containers/Congratulations', {
...
describe('<Congratulations />', () => {
  it('renders the Congratulations component', () => {
    ...
    expect(wrapper.childAt(2).prop('children')[0]).to.equal(TEST_KID.name.split(' ')[0]);
    expect(wrapper.childAt(3).prop('children')[1]).to.equal(TEST_KID.name.split(' ')[0]);
  });
